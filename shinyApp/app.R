...
server <- function(input, output) {
   ...
   output$co2History <- renderPlot({
     ...
     ggplot(co2Data, aes(x=year, y=CO2)) + geom_smooth(method = "loess", span=0.005, se=FALSE, col = "darkgreen") +
   ...
   output$heatDays <- renderPlot({
     ...
     ggplot(plotTab, aes(x=x,y=y)) + geom_point(shape =21, fill = plotTab$col, alpha =0.8, size = plotTab$days+5) +
       ...
       theme_void() + theme(plot.title = element_text(size = 20, hjust =0.5, face = "bold"))
   })
}
